...
unsigned int hash(char *key) 
{
	...
	int x;
	//loop through the values of the key then add them up
	for(x = 0; x < strlen(key); x ++){
	  hashvalue = hashvalue + (int)key[x];
	}
	hashvalue = hashvalue % BINSIZE;// mod the sum of the hasvalues
...
struct binEntry *lookup(char *key)
{
	...
	entry = hashtable[hValue];
	
	while(entry != NULL){// loop though the list in the hastable[value]
	  if(strcmp(entry->key, key)==0){
	    return entry;
	  }
	  entry = entry->next;// go to the next value in the linked list
	}
...
void drop(char *key)
{
  struct binEntry *holder;
  
  if((holder = lookup(key)) == 0){//check to make sure that the value isnt NULL
      return;
    }

  //first node in the list
  if(holder->next != NULL && holder->prev == NULL){
    holder->next->prev = NULL;
    hashtable[hash(key)] = holder->next;
  }
  // only node in the list
  else if(holder->next == NULL && holder->prev == NULL){
